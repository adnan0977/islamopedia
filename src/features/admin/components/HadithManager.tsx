  ...
  const handleSyncFromApi = async () => {
    ...
    try {
      ...
      apiBooks.forEach((book: HadithApiBook) => {
        ...
        batch.set(bookRef, {
          ...book,
          id: book.bookSlug, // Use slug as ID for better SEO/URLs
          isActive: true,
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-zinc-950 p-8 rounded-3xl border border-zinc-900 shadow-xl">
        <div className="space-y-2 text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-3">
            ...
            <h2 className="text-2xl font-headline font-bold text-white">Hadith Book Registry</h2>
          </div>
          <p className="text-sm text-zinc-500 font-medium">Connect and manage primary source collections from HadithAPI.</p>
        </div>
            ...
            <Card key={book.id} className="bg-zinc-950 border-zinc-900 rounded-[2rem] overflow-hidden group hover:border-zinc-700 transition-all flex flex-col shadow-2xl">
              ...
              <CardContent className="p-8 flex-1 space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 text-zinc-400">
                    ...
                    <span className="text-xs font-bold">{book.chapters_count} Chapters Indexed</span>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-zinc-900 flex items-center justify-between">
                  <div className="flex items-center gap-2 bg-zinc-900 px-3 py-1.5 rounded-full border border-zinc-800">
                    ...
                    <span className="text-[10px] font-black uppercase text-zinc-400">{parseInt(book.hadiths_count).toLocaleString()} Items</span>
                  </div>
              ...
              <CardFooter className="p-6 bg-zinc-900/10 border-t border-zinc-900 flex justify-end gap-2">
                <Button 
                  ...
                  onClick={() => deleteDocumentNonBlocking(doc(db, 'had_books', book.id))}
                >
          ...
          {(!books || books.length === 0) && (
            <div className="col-span-full py-32 text-center bg-zinc-950/30 rounded-[2.5rem] border-2 border-dashed border-zinc-900 space-y-6">
              ...
              <div className="space-y-2">
                <p className="text-zinc-500 font-medium">The book registry is currently empty.</p>
                <p className="text-xs text-zinc-700 uppercase font-black tracking-widest">Click 'Sync Collections' to fetch from HadithAPI</p>
              </div>
            </div>
          )}
        </div>
