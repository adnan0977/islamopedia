...
export function AppSettings() {
  ...
  const seedLanguages = async () => {
    ...
    try {
      ...
      ISO_LANGUAGES.forEach(([name, code]) => {
        const langRef = doc(db, 'languages', code);
        batch.set(langRef, { id: code, name, isoCode: code }, { merge: true });
      });
      await batch.commit();
      toast({ title: "Registry Updated", description: "Successfully seeded 180+ ISO language mappings." });
    } catch (e: any) {
        ...
        <CardHeader className="p-8 border-b border-zinc-900 bg-zinc-900/20">
          <div className="flex items-center justify-between">
            ...
            <Button 
              ...
              className="rounded-xl border-zinc-800 text-zinc-400 font-bold hover:bg-zinc-900"
            >
